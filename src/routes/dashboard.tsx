          ...
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex gap-1.5">
              {["12M", "6M", "30D", "7D"].map((p, i) => (
                <button key={p} className={`text-xs px-3 py-1.5 rounded-lg transition-all ${i === 0 ? "bg-gradient-primary text-primary-foreground shadow-sm shadow-primary/25" : "border border-border text-muted-foreground hover:bg-accent hover:text-foreground hover:border-primary/30"}`}>{p}</button>
              ))}
            </div>
          </div>
        ...
        <div className="glass-card rounded-2xl p-6">
          ...
          <div className="space-y-3">
            {aiInsights.map((i, idx) => {
              ...
              const color = i.type === "warning" ? "text-warning bg-warning/12 border-warning/20" : i.type === "opportunity" ? "text-primary bg-primary/12 border-primary/20" : "text-success bg-success/12 border-success/20";
              return (
                <div key={idx} className="rounded-xl border border-border/50 p-3 hover:border-primary/35 hover:bg-accent/30 transition-all cursor-pointer">
                  <div className="flex items-start gap-3">
                    <div className={`h-8 w-8 rounded-lg border flex items-center justify-center shrink-0 ${color}`}><Icon className="h-4 w-4" /></div>
                    <div className="min-w-0 flex-1">
                      <p className="text-sm font-medium">{i.title}</p>
                      <p className="text-xs text-muted-foreground mt-1 leading-relaxed">{i.body}</p>
                      <div className="flex items-center gap-2 mt-2">
                        <div className="flex-1 h-1 rounded-full bg-secondary overflow-hidden">
                          <div className="h-full bg-gradient-primary rounded-full" style={{ width: `${i.confidence * 100}%` }} />
                        </div>
                        <span className="text-[10px] text-muted-foreground shrink-0">{Math.round(i.confidence * 100)}% conf.</span>
                      </div>
                    </div>
                  </div>
                </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mt-4">
        ...
        <div className="glass-card rounded-2xl p-6">
          ...
          <div className="space-y-1">
            {notifications.map((n) => {
              const colors: Record<string, string> = { success: "bg-success shadow-[0_0_6px_var(--color-success)]", warning: "bg-warning shadow-[0_0_6px_var(--color-warning)]", info: "bg-primary shadow-[0_0_6px_var(--color-primary)]" };
              return (
                <div key={n.id} className="flex gap-3 p-3 rounded-xl hover:bg-accent/40 transition-all cursor-pointer group">
