...
export function Logo({ className = "" }: { className?: string }) {
  return (
    <Link to="/" className={`flex items-center gap-2.5 font-semibold ${className}`}>
      <span className="relative inline-flex h-8 w-8 items-center justify-center rounded-lg bg-gradient-primary shadow-glow shrink-0">
      ...
      <span className="text-lg tracking-tight leading-none">
        DataVision<span className="text-gradient font-bold"> AI</span>
      </span>
    </Link>
  );
}
